...
Use this shape for later “who takes X” pages. Page one (`paint.html`) and page three (`mattresses.html`) are the examples. Do not invent hours, fees, caps, addresses, or phone numbers. If an official URL disagrees with the page, prefer the official page and update the last-checked date.

## File

- One HTML file per topic (repo root, next to `index.html`).
- Shared `style.css`. Shared nav: Home, Paint, Mattresses, Coming.
...
2. **Nav** — Home · Paint · Mattresses · Coming (`aria-current="page"` on the current item).
...
Write the date the official URLs were actually read, like `29 Aug 2026` or `30 Aug 2026`. Same date in the header, the home directory line, the site table, and the footer. Re-read official pages before changing a fact.

## Do not

- Write remaining later pages (transfer-station hours, TVs / e-waste, Freon, tires / propane, HHW how-to) until each one is researched.
